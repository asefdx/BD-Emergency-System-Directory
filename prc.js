...
// attach hearts for 9 cards
heartClick("heart-btn1");
...
function renderHistory() {
  ...
  for (const historyData of callHistoryData) {
    ...
    div.innerHTML = `
      <div class="flex justify-between items-center bg-[#fafafa] mb-2 p-3 rounded-[8px] shadow">
        <div>
          <h1 class="font-[600] inter">${historyData.name}</h1>
          <p class="text-[#5c5c5c]">${historyData.number}</p>
        </div>
        <p class="text-sm">${historyData.date}</p>
      </div>
    `;
...
// clear history (attached once)
document.getElementById("clear-btn").addEventListener("click", function () {
  callHistoryData.length = 0;
  renderHistory();
});

// register all call buttons
forCall("call-btn1", "national-emergency-number", "national-emergency-number-call");
...
forCall("call-btn6", "anti-corruption-helpline", "anti-corruption-helpline-call");
forCall("call-btn7", "electricity-helpline", "electricity-helpline-call");
forCall("call-btn8", "brac-helpline", "brac-helpline-call");
forCall("call-btn9", "bangladesh-railway-helpline", "bangladesh-railway-helpline-call");
...
function forCopy(id1, id2, id3) {
  ...
  btn.addEventListener("click", function () {
    ...
    let updatedCopyCount = copyCount + 1;
